
import { Question } from './types';

export const angleQuestions: Question[] = [
  {
    id: 1,
    image_url: '/img/angles/Helado.png',
    question: "Fíjate en la punta de este cono de helado, ¿Qué ángulo es?",
    imageDescription: "Imagen de un cono de helado cuya punta forma un ángulo agudo",
    options: ["Ángulo agudo", "Ángulo recto", "Ángulo obtuso", "Ángulo llano"],
    correctAnswer: "Ángulo agudo"
  },
  {
    id: 2,
    image_url: '/img/angles/Lapiz.png',
    question: "¿Qué ángulo ves en el extremo de este lápiz?",
    imageDescription: "Imagen de la punta de un lápiz que forma un ángulo agudo",
    options: ["Ángulo agudo", "Ángulo recto", "Ángulo obtuso", "Ángulo llano"],
    correctAnswer: "Ángulo agudo"
  },
  {
    id: 3,
    image_url: '/img/angles/Silla.png',
    question: "El respaldo de esta silla y el asiento, ¿Qué ángulo forman?",
    imageDescription: "Imagen de una silla donde el respaldo y el asiento forman un ángulo recto",
    options: ["Ángulo agudo", "Ángulo recto", "Ángulo obtuso", "Ángulo llano"],
    correctAnswer: "Ángulo recto"
  },
  {
    id: 4,
    image_url: '/img/angles/Grapadora.png',
    question: "¿Cómo es el ángulo de la engrapadora en esta imagen?",
    imageDescription: "Imagen de una engrapadora abierta formando un ángulo obtuso",
    options: ["Ángulo agudo", "Ángulo recto", "Ángulo obtuso", "Ángulo llano"],
    correctAnswer: "Ángulo agudo"
  },
  {
    id: 5,
    image_url: '/img/angles/Semaforo.png',
    question: "¿Qué ángulo hay entre el poste y el semáforo?",
    imageDescription: "Imagen de un semáforo con un poste formando un ángulo recto",
    options: ["Ángulo agudo", "Ángulo recto", "Ángulo obtuso", "Ángulo llano"],
    correctAnswer: "Ángulo recto"
  },
  {
    id: 6,
    image_url: '/img/angles/Escalera.png',
    question: "¿Qué tipo de ángulo hay entre la escalera y el suelo?",
    imageDescription: "Imagen de una escalera apoyada en la pared formando un ángulo recto con el suelo",
    options: ["Ángulo agudo", "Ángulo recto", "Ángulo obtuso", "Ángulo llano"],
    correctAnswer: "Ángulo recto"
  },
  {
    id: 7,
    image_url: '/img/angles/Columpio.png',
    question: "Fíjate en el brazo de este columpio, ¿Qué tipo de ángulo está formando?",
    imageDescription: "Imagen de un columpio con el brazo formando un ángulo obtuso",
    options: ["Ángulo agudo", "Ángulo recto", "Ángulo obtuso", "Ángulo llano"],
    correctAnswer: "Ángulo recto"
  },
  {
    id: 8,
    image_url: '/img/angles/Avion.png',
    question: "¿Qué ángulo forman las alas de este avión de papel?",
    imageDescription: "Imagen de un avión de papel con sus alas formando un ángulo agudo",
    options: ["Ángulo agudo", "Ángulo recto", "Ángulo obtuso", "Ángulo llano"],
    correctAnswer: "Ángulo agudo"
  },
  {
    id: 9,
    image_url: '/img/angles/Rampa.png',
    question: "Observa esta rampa, ¿Qué ángulo forma con el piso?",
    imageDescription: "Imagen de una rampa formando un ángulo agudo con el piso",
    options: ["Ángulo agudo", "Ángulo recto", "Ángulo obtuso", "Ángulo llano"],
    correctAnswer: "Ángulo agudo"
  },
  {
    id: 10,
    image_url: '/img/angles/Sofa.png',
    question: "¿Qué ángulo tiene el asiento de esta silla?",
    imageDescription: "Imagen de una esquina de un sofá formando un ángulo recto",
    options: ["Ángulo agudo", "Ángulo recto", "Ángulo obtuso", "Ángulo llano"],
    correctAnswer: "Ángulo recto"
  },
  {
    id: 11,
    image_url: '/img/angles/Reloj.png',
    question: "¿Qué ángulo forman las manecillas del reloj?",
    imageDescription: "Imagen de reloj formando un ángulo obtuso con sus manecillas",
    options: ["Ángulo agudo", "Ángulo recto", "Ángulo obtuso", "Ángulo llano"],
    correctAnswer: "Ángulo obtuso"
  },
  {
    id: 12,
    image_url: '/img/angles/Pizza.png',
    question: "¿Qué ángulo forma esta rebanada de pizza?",
    imageDescription: "Imagen de una rebanada de pizza formando un ángulo agudo",
    options: ["Ángulo agudo", "Ángulo recto", "Ángulo obtuso", "Ángulo llano"],
    correctAnswer: "Ángulo agudo"
  },
  {
    id: 13,
    image_url: '/img/angles/Libro.png',
    question: "¿Qué ángulo forma este libro abierto?",
    imageDescription: "Imagen de un libro abierto formando un ángulo llano",
    options: ["Ángulo agudo", "Ángulo recto", "Ángulo obtuso", "Ángulo llano"],
    correctAnswer: "Ángulo llano"
  },
  {
    id: 14,
    image_url: '/img/angles/Puerta.png',
    question: "¿Qué ángulo se forma con la apertura de esta puerta",
    imageDescription: "Imagen un par de puertas abiertas formando un ángulo obtuso",
    options: ["Ángulo agudo", "Ángulo recto", "Ángulo obtuso", "Ángulo llano"],
    correctAnswer: "Ángulo obtuso"
  },
  {
    id: 15,
    image_url: '/img/angles/Mesa.png',
    question: "¿Qué ángulo tienen las esquinas de esta mesa?",
    imageDescription: "Imagen de una Mesa cuadrada, sus esquinas forman un ángulo recto",
    options: ["Ángulo agudo", "Ángulo recto", "Ángulo obtuso", "Ángulo llano"],
    correctAnswer: "Ángulo recto"
  }
];
